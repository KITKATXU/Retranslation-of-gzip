...
# 122 "util.c"
    ...
    while (insize > inptr) {
        ...
        got = read_buffer (in, (char *) inbuf, 0x8000);
        if (got == -1)
            read_error();
        bytes_in += got;
        insize = (unsigned)got;
    ...
    int eof_ok;
{
    ...
    insize = 0;
    do {
        len = read_buffer (ifd, (char *) inbuf + insize, 0x8000 - insize);
        if (len == 0) break;
        if (len == -1) {
          read_error();
          break;
...
# 339 "util.c"
          ? tolower ((unsigned char) *t) : ((unsigned char) *t));
    return s;
}
...
# 513 "util.c" 3
   rpl_fprintf
# 513 "util.c"
          (file, "%5.1f%%", den == 0 ? 0 : 100.0 * num / den);
}
